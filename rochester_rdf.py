...
ROOT.gInterpreter.Declare("""
    ROOT::VecOps::RVec<Int_t> get_indices(UInt_t nMuon, ROOT::VecOps::RVec<Float_t> *Muon_pt, ROOT::VecOps::RVec<Float_t> *Muon_eta, ROOT::VecOps::RVec<Float_t> *Muon_phi, ROOT::VecOps::RVec<Float_t> *Muon_mass, ROOT::VecOps::RVec<Float_t> *Muon_tkRelIso, ROOT::VecOps::RVec<Bool_t> *Muon_mediumId){
        Float_t deltaM = 1000;
        Int_t ind1, ind2;
        ind1 = -99;
        ind2 = -99;
...
def make_ntuples(samples):
    datadir = "/ceph/jdriesch/rochester/"

    for s in samples[:2]:
        quants = [
            "pt_Z", "mass_Z", "eta_Z", "phi_Z",
            "pt_1", "mass_1", "eta_1", "phi_1",
            "pt_2", "mass_2", "eta_2", "phi_2",
        ]
        # load nanoAOD
        rdf = ROOT.RDataFrame("Events", f"{datadir}/{s}.root")
        
        # only collect events w/ >1 muon and find muon pair closest to z mass
        rdf = rdf.Filter("Muon_pt.size() > 1")
        rdf = rdf.Define("ind", "ROOT::VecOps::RVec<Int_t> (get_indices(nMuon, &Muon_pt, &Muon_eta, &Muon_phi, &Muon_mass, &Muon_tkRelIso, &Muon_mediumId))")
        ...
        rdf.Snapshot("Events", f"{datadir}/{s}_ntuple.root", quants)
...
def hist_zpt(samples, pt_bins):
    ...
    for s in samples[:2]:
        ...
        rdf = ROOT.RDataFrame("Events", f"{datadir}/{s}_ntuple.root")
